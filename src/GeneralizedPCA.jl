...
Given the eigendecomposition of the ``n × n`` covariance matrix,
``Σ = UΛUᵀ``, with eigenvalues sorted in descending order, i.e.
``λ₁ ≥ λ₂ ⋯ ≥ λₙ``, the first ``m`` components are selected according
to one or more of the criteria listed above.

If ``m = n``, then we have the canonical PCA whitening matrix,
 ``W = Λ^{-\frac{1}{2}}Uᵀ``. Otherwise, for ``m < n``,
...
from the upper left of ``Λ`` and the ``m × n`` matrix from the
top of ``Uᵀ``. The inverse transform is then formed by
selecting the ``n × m`` matrix from the left of ``U`` and the same matrix
from ``Λ``.
"""
